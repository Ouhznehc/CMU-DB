...
namespace bustub {
  ...
  public:
    ...
    std::unique_ptr<TrieNode>* InsertChildNode(char key_char, std::unique_ptr<TrieNode>&& child) {
      ...
      auto ret = this->children_.emplace(key_char, std::move(child));
    ...
    std::unique_ptr<TrieNode>* GetChildNode(char key_char) {
      ...
      return &this->children_.find(key_char)->second;
    }
  ...
  public:
    ...
    bool Insert(const std::string& key, T value) {
      ...
      std::unique_ptr<TrieNode>* father = cur;
      for (char ch : key) {
        father = cur;
        if (!(*father)->HasChild(ch)) {
          (*father)->InsertChildNode(ch, std::make_unique<TrieNode>(ch));
        }
      ...
      char last_key = key[key.size() - 1];
      std::unique_ptr<TrieNode> end_node = std::make_unique<TrieNodeWithValue<T>>(std::move(**cur), value);
      (*father)->RemoveChildNode(last_key);
      (*father)->InsertChildNode(last_key, std::move(end_node));
      this->latch_.WUnlock();
      return true;
    }
    ...
    bool Remove(const std::string& key) {
      ...
      std::stack<std::unique_ptr<TrieNode>*> path;
      ...
      while (!path.empty()) {
        ...
        auto child = (*parent)->GetChildNode(key_char);

        if ((*child)->IsEndNode() || (*child)->HasChildren()) {
          break;
        }
    ...
    T GetValue(const std::string& key, bool* success) {
      ...
      if (cur->IsEndNode()) {
        auto end_node = dynamic_cast<TrieNodeWithValue<T> *>(cur);
        if (end_node) {
          *success = true;
          this->latch_.RUnlock();
